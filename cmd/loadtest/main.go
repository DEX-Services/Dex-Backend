...
// BIUSDB, then drives a real scenario matrix across all 17 live markets:
...
var futuresMarkets = []market{
	{"BTC", "FUTURES", "BTC-BIUSDB"},
	{"ETH", "FUTURES", "ETH-BIUSDB"},
	{"SOL", "FUTURES", "SOL-BIUSDB"},
	{"BNB", "FUTURES", "BNB-BIUSDB"},
	{"EURUSD", "FUTURES", "EURUSD-BIUSDB"},
	{"GBPUSD", "FUTURES", "GBPUSD-BIUSDB"},
	{"AUDUSD", "FUTURES", "AUDUSD-BIUSDB"},
	{"GOLD", "FUTURES", "GOLD-BIUSDB"},
	{"SILVER", "FUTURES", "SILVER-BIUSDB"},
	{"CrudeOIL", "FUTURES", "CrudeOIL-BIUSDB"},
	{"AAPL.us", "FUTURES", "AAPL.us-BIUSDB"},
	{"TSLA.us", "FUTURES", "TSLA.us-BIUSDB"},
	{"NVDA.us", "FUTURES", "NVDA.us-BIUSDB"},
}
...
func main() {
	...
	biusdFund := flag.String("fund", "100000", "BIUSDB credited to each user")
	...
	if *phase == "all" || *phase == "users" {
		...
		for _, uid := range users {
			...
			go func(uid string) {
				...
				if err := creditBIUSDB(adminTok, uid, *biusdFund); err != nil {
					atomic.AddInt64(&errCount, 1)
...
func runScenarios(users []string, tokens map[string]string) {
	...
	// 2. Spot limit order BUY/SELL, crossing + resting
	{
		status, body, _ := postTrade(t[u[1]], "/trade/order", tradeOrderRequest{
			Symbol: "ETH-BIUSDB", Market: "SPOT", Side: "BUY", Type: "LIMIT", Price: "100000", Qty: "0.01", // crosses (way above market)
		})
		...
		status2, body2, _ := postTrade(t[u[1]], "/trade/order", tradeOrderRequest{
			Symbol: "ETH-BIUSDB", Market: "SPOT", Side: "SELL", Type: "LIMIT", Price: "1", Qty: "0.01", // rests far below
		})
	...
	{
		user := u[3]
		s1, b1, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "FUTURES", Side: "BUY", Type: "MARKET", Qty: "0.005", Leverage: &lev, MarginMode: "ISOLATED"})
		record("cross-asset-user-BTCfut", s1 == 200, fmt.Sprintf("body=%v", trimAny(b1)))
		s2, b2, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "GOLD-BIUSDB", Market: "FUTURES", Side: "BUY", Type: "MARKET", Qty: "0.01", Leverage: &lev, MarginMode: "ISOLATED"})
		record("cross-asset-user-GOLDfut", s2 == 200, fmt.Sprintf("body=%v", trimAny(b2)))
		s3, b3, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "AAPL.us-BIUSDB", Market: "FUTURES", Side: "BUY", Type: "MARKET", Qty: "0.01", Leverage: &lev, MarginMode: "ISOLATED"})
		record("cross-asset-user-AAPLfut", s3 == 200, fmt.Sprintf("body=%v", trimAny(b3)))
		s4, b4, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "0.001"})
	...
	{
		user := u[10]
		s1, b1, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "999999"})
		record("edge-insufficient-balance", s1 >= 400, fmt.Sprintf("status=%d body=%v", s1, trimAny(b1)))

		s2, b2, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "FAKE-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "1"})
		record("edge-invalid-symbol", s2 >= 400, fmt.Sprintf("status=%d body=%v", s2, trimAny(b2)))

		s3, b3, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "0"})
		record("edge-zero-qty", s3 >= 400, fmt.Sprintf("status=%d body=%v", s3, trimAny(b3)))

		s4, b4, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "-1"})
		record("edge-negative-qty", s4 >= 400, fmt.Sprintf("status=%d body=%v", s4, trimAny(b4)))

		s5, b5, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "SPOT", Side: "SELL", Type: "MARKET", Qty: "50"})
		record("edge-sell-more-than-held", s5 >= 400, fmt.Sprintf("status=%d body=%v", s5, trimAny(b5)))

		badLev := 500
		s6, b6, _ := postTrade(t[user], "/trade/order", tradeOrderRequest{Symbol: "BTC-BIUSDB", Market: "FUTURES", Side: "BUY", Type: "MARKET", Qty: "0.01", Leverage: &badLev, MarginMode: "ISOLATED"})
		...
		body := tradeOrderRequest{Symbol: "ETH-BIUSDB", Market: "SPOT", Side: "BUY", Type: "MARKET", Qty: "0.002"}
		for i := 0; i < 2; i++ {
			wg.Add(1)
			go func(i int) {
				defer wg.Done()
