...
func (r *AdminRepo) Summary(ctx context.Context) (models.AdminSummary, error) {
	...
	if err := r.pool.QueryRow(ctx, `
		SELECT
			...
			(SELECT COALESCE((SELECT available_raw FROM p2p_admin_wallet_balances WHERE asset='BI2XUSD'), 0)::text)`).
	...
	tokenRows, err := r.pool.Query(ctx, `
		...
		FROM (
			SELECT 'BI2XUSD' AS token, COALESCE(SUM("BI2XUSD"), 0) AS amount, COALESCE(SUM("BI2XUSD_locked"), 0) AS locked FROM user_balances
			...
			UNION ALL SELECT 'BI2X', COALESCE(SUM("BI2X"), 0), COALESCE(SUM("BI2X_locked"), 0) FROM user_balances
		) totals`)
	if err != nil {
		return s, err
	}
