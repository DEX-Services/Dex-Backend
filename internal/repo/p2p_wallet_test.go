...
func TestP2PWalletEscrowSuccessAndRefund(t *testing.T) {
	...
	if err := ledger.CreditBalance(ctx, sellerID, "USDB", "20200000"); err != nil {
...
func TestP2PWalletUSDBEscrowSuccess(t *testing.T) {
	...
	if err := ledger.CreditBalance(ctx, sellerID, "USDB", "10100000"); err != nil {
		t.Fatalf("credit seller USDB: %v", err)
	}
	balance, moved, err := p2p.FundWalletAsset(ctx, sellerID, "USDB", "10100000", "fund-usdb-test-0001")
	if err != nil || !moved {
	...
	assertWallet(t, buyerUSDB, "9900000", "0", "9900000")
...
func TestP2PBuyAdUsesTakerAsSeller(t *testing.T) {
	...
	if err := ledger.CreditBalance(ctx, takerSellerID, "USDB", "5050000"); err != nil {
		t.Fatalf("credit taker seller: %v", err)
	}
	if _, moved, err := p2p.FundWalletAsset(ctx, takerSellerID, "USDB", "5050000", "fund-buy-ad-seller"); err != nil || !moved {
...
func TestP2POrderEvidenceChatAndAppealResolution(t *testing.T) {
	...
	if _, moved, err := p2p.FundWalletAsset(ctx, sellerID, "USDB", "20200000", "workflow-fund"); err != nil || !moved {
	...
	createOrder := func(key string) *models.P2POrder {
		listing, err := p2p.CreateListingWithDetails(ctx, sellerID, "SELL", "USDB", "10000000", []string{"UPI"}, "")
		if err != nil {
			t.Fatalf("create listing: %v", err)
		}
...
func TestP2PListingLimitsAndAdvertiserStatistics(t *testing.T) {
	...
	if _, moved, err := p2p.FundWalletAsset(ctx, sellerID, "USDB", "10100000", "limits-fund"); err != nil || !moved {
		t.Fatalf("fund seller P2P wallet: moved=%v err=%v", moved, err)
	}
...
func assertAdminP2PWallet(t *testing.T, pool *pgxpool.Pool, want string) {
	t.Helper()
	var got string
	if err := pool.QueryRow(context.Background(), `SELECT available_raw::text FROM p2p_admin_wallet_balances WHERE asset='USDB'`).Scan(&got); err != nil {
